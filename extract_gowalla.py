...
traj = pd.read_csv('./Gowalla.txt', sep='\t', names=['user', 'check_in_time', 'latitude', 'longitude', 'location_id'])
...
poi_info.to_csv('./gowalla.geo', index=False)

# 处理 usr
usr_info = pd.unique(traj['user'])
usr_info = pd.DataFrame(usr_info, columns=['usr_id'])
# 因为没有 properties 信息所以直接存吧
usr_info.to_csv('./gowalla.usr', index=False)

# 但是有 rel
rel = pd.read_csv('./Gowalla_edges.txt', sep='\t', names=['origin_id', 'destination_id'])
...
dyna.to_csv('./gowalla.dyna', index=False)


dyna = pd.read_csv('./gowalla.dyna')

group_location = dyna.groupby('location').count()
filter_location = group_location[group_location['time'] > 5]
location_index = filter_location.index.tolist()
