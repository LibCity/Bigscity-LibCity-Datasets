...
word_index = pd.read_csv('./word_index.txt', sep='\t', names=['index', 'word'])

# poi 是 id， text_content 是由 word index 组成的句子，啊这我要给他还原吗，还是给他还原？
# 还原吧
train = pd.read_csv('./train.txt', sep='\t', names=['user', 'lat', 'lng', 'timestamp', 'poi', 'text_content'])
test = pd.read_csv('./test.txt', sep='\t', names=['user', 'lat', 'lng', 'timestamp', 'poi', 'text_content'])
validation = pd.read_csv('./validation.txt', sep='\t', names=['user', 'lat', 'lng', 'timestamp', 'poi', 'text_content'])
...
# poi.to_csv('./instagram/instagram.geo', index=False)

# # 做 usr 表
# user = pd.unique(total_check_in['user'])
# user = pd.DataFrame(user, columns=['usr_id'])
# user.to_csv('./instagram/instagram.usr', index=False)
...
total_check_in.to_csv('./instagram.dyna', index=False)
