...
outputdir = 'output/METR_LA'
util.ensure_dir(outputdir)

dataurl = 'input/METR-LA/'
dataname = outputdir+'/METR_LA'

dataset = pd.read_csv(dataurl+'sensor_graph/graph_sensor_locations.csv')
idset = set()
geo = []
...
rel = []
reldict = dict()
dataset = pd.read_csv(dataurl+'sensor_graph/distances_la_2012.csv')
for i in range(dataset.shape[0]):
    sid = dataset['from'][i]
    eid = dataset['to'][i]
...
# dataset = h5py.File(dataurl+'metr-la.h5', 'r')
# df = dataset['df']
...
df = pd.read_hdf(dataurl+'metr-la.h5')
...
config['info']['data_files'] = ['METR_LA']
