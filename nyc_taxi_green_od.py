...
outputdir = 'output/NYC_TAXI_Green_od'
util.ensure_dir(outputdir)

dataurl = 'input/NYC-Taxi/NYC-Taxi 2020-06/green_tripdata_2020-06.csv'
lookupurl = 'input/NYC-Taxi/taxi_zones/taxi_zones.shp'
dataname = outputdir+'/NYC_TAXI_Green'
...
# dyna_time = []
# for i in range(dataset.shape[0]):
#     stime = util.add_TZ(dataset['lpep_pickup_datetime'][i])
#     etime = util.add_TZ(dataset['lpep_dropoff_datetime'][i])
#     dyna_time.append(str(stime)+' '+str(etime))

dyna_time = []
for i in range(dataset.shape[0]):
    stime = dataset['lpep_pickup_datetime'][i].replace(' ', 'T')+'Z'
    etime = dataset['lpep_dropoff_datetime'][i].replace(' ', 'T')+'Z'
    dyna_time.append(str(stime)+' '+str(etime))
    if i % 10000 == 0:
...
dyna = dyna.drop('lpep_pickup_datetime', axis=1)
dyna = dyna.drop('lpep_dropoff_datetime', axis=1)
...
config['dyna']['od'] = {'entity_id': 'usr_id', 'od_id': 'rel_id',
                        ...
                        'store_and_fwd_flag': 'other', 'payment_type': 'enum',
                        'fare_amount': 'num', 'extra': 'num', 'mta_tax': 'num',
                        'tip_amount': 'num', 'tolls_amount': 'num', 'trip_type': 'enum',
                        'improvement_surcharge': 'num', 'total_amount': 'num',
                        'congestion_surcharge': 'num', 'ehail_fee': 'other'}
